...
/*****************************************************************
 *  allocates the LoShade GCs
 ****************************************************************/
void allocateLoGc(char* fore, char* back, char* fore2)
{
  XGCValues mygcv;
  unsigned long mygcm;
  int i;
  ...
  Pixel forePixel,backPixel,fore2Pixel;
  fvwm_msg(WARN,"allLOGC:fore",fore);
  fvwm_msg(WARN,"allLOGC:back",back);
 
  if(Scr.d_depth > 2)
    {
      forePixel=GetColor(fore);
      backPixel=GetColor(back);
      fore2Pixel=GetColor(fore2);
    }
  else {
      forePixel=GetColor(white);
      backPixel=GetColor(black);
      fore2Pixel=GetColor(black);  
  }
  
  mygcm = GCForeground|GCBackground|GCFunction|GCPlaneMask|GCGraphicsExposures|GCLineWidth;;
  mygcv.foreground = backPixel;
  mygcv.background = forePixel;
  mygcv.line_width = 0;
  mygcv.function = GXcopy;
  mygcv.plane_mask = AllPlanes;
  mygcv.graphics_exposures = False;

  for(i=0; i<BLOCKSNUMBER; ++i)
    {
      mygcv.foreground = MergeColors(mygcv.foreground,
				     fore2Pixel,
				     i);
      XChangeGC(dpy,Scr.LoShades[i],mygcm,&mygcv);
    }
}
...
void SetHiColor(XEvent *eventp,Window w,FvwmWindow *tmp_win,
  ...
  {
  XGCValues gcv, mygcv;
  unsigned long gcm, mygcm;
  int i;
  char *hifore=NULL, *hiback=NULL, *hifore2=NULL;
  extern char *white, *black;
  FvwmWindow *hilight;
  
  action = GetNextToken(action, &hifore);
  action = GetNextToken(action, &hiback); 
  GetNextToken(action, &hifore2);

  if(hifore2 != NULL) 
    {
      if( strcmp(hifore2,"")==0 )
	active_has_shading=0;
      else active_has_shading=1;
    }
  ...
  if(Scr.d_depth > 2)
    ...
    if(hifore2 != NULL) Scr.ActiveTitleColors.fore2 = GetColor(hifore2);
   }
  else
    {
    Scr.ActiveTitleColors.back  = GetColor(white);
    Scr.ActiveTitleColors.fore  = GetColor(black); 
    Scr.ActiveTitleColors.fore2 = GetColor(white);
    }

  fvwm_msg(WARN,"setHi","Just instantiated colors");
  ...
  /* I guess now it is the time to allocate the shading GCs */

  mygcm = GCForeground|GCBackground|GCFunction|GCPlaneMask|GCGraphicsExposures|GCLineWidth;;  
  mygcv.foreground = Scr.ActiveTitleColors.back;
  mygcv.background = Scr.ActiveTitleColors.fore2;
  mygcv.line_width = 0;
  mygcv.function = GXcopy;
  mygcv.plane_mask = AllPlanes;
  mygcv.graphics_exposures = False;
  ...
  for(i=0; i<BLOCKSNUMBER; ++i)
    {
     mygcv.foreground = MergeColors(mygcv.foreground,
				    Scr.ActiveTitleColors.fore2,
				    i);
     XChangeGC(dpy,Scr.HiShades[i],mygcm,&mygcv);
    }

  if (hifore)  free(hifore);
  if (hifore2) free(hifore2);
