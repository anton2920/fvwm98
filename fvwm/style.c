...
char *_forecolor=NULL, *_forecolor2=NULL, *_backcolor=NULL;
int active_has_shading=0;
int normal_has_shading=0;
  ...
  while((*restofline != 0)&&(*restofline != '\n'))
  {
    ...
    switch (tolower(restofline[0]))
    {
      ...
      case 'e':
        break;
      case 'f':
        if(strncasecmp(restofline,"ForeColor2",10)==0)
        {
          restofline +=10;
          while(isspace(*restofline))restofline++;
          tmp = restofline;
          ...
          if(len > 0)
          {
            forecolor2 = safemalloc(len+1);
            strncpy(forecolor2,restofline,len);
            forecolor2[len] = 0;
	    off_flags |= FORE2_COLOR_FLAG;
	    normal_has_shading=1;
	  }
          restofline = tmp;
        }
        else if(strncasecmp(restofline,"ForeColor",9)==0)
	  {
	    restofline +=9;
	    while(isspace(*restofline))restofline++;
	    tmp = restofline;
  ...
  if(off_flags & BACK_COLOR_FLAG)
    {
      ...
      if(_forecolor!=NULL && _forecolor2!=NULL)
	allocateLoGc(_forecolor,_backcolor,_forecolor2);
    }	
  else if(off_flags & FORE_COLOR_FLAG)
    {
      _forecolor=forecolor;
      if(_backcolor!=NULL && _forecolor2!=NULL)
        allocateLoGc(_forecolor,_backcolor,_forecolor2);
    } 
  if(off_flags & FORE2_COLOR_FLAG)
    {
      _forecolor2=forecolor2;
      if(_backcolor!=NULL && _forecolor!=NULL)
        allocateLoGc(_forecolor,_backcolor,_forecolor2);
    }
