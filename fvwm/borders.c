...
void SetTitleBar (FvwmWindow *t, Bool onoroff, Bool NewTitle)
  ...
  if(onoroff)
    {
      if(active_has_shading)
	{/*active shade*/
	  for(myt=0;myt<BLOCKSNUMBER;++myt)
	    XFillRectangle(dpy,
			   t->title_w,
			   Scr.HiShades[myt],
			   x - 2 + (myt*(t->title_width/BLOCKSNUMBER)), 
			   0, 
			   (myt+1)*(t->title_width/BLOCKSNUMBER), 
			   t->title_height);
