  ...
  while(1)
      ...
      if(My_XNextEvent(dpy,&Event))
	{
	  switch(Event.type)
		...
		i=0;
		while(tmp != NULL){
		  if (Event.xproperty.window == (unsigned long)tmp->id)
		    break;
		  tmp = tmp->next;
		  ++i;
		}
...
void process_message(unsigned long type, unsigned long *body)
{
  ...
  case M_CONFIGURE_WINDOW:
    if (ready){
      ...
      tmp = Head;
      while(tmp != NULL){
	if ((unsigned long)tmp->id == body[0]){
	  if (((unsigned long)tmp->desk != body[7]) && !(tmp->flags & STICKY)){
	    olddesk = tmp->desk;
	    tmp->desk = body[7];
  ...
  case M_FOCUS_CHANGE:
    ...
    tmp = Head;
    while(tmp != NULL){
      if ((unsigned long)tmp->id == body[0]) break;
      tmp = tmp->next;
    }
    old = Hilite;
    Hilite = tmp;
    ...
    break;
  case M_NEW_DESK:
    if ((unsigned long)CurrentDesk != body[0]){
      CurrentDesk = body[0];
...
Bool DeleteItem(unsigned long id)
{
  ...
  if ((unsigned long)tmp->id == id){
    ...
    if ((unsigned long)tmp->IconWin != None)
...
void ExecuteAction(int x, int y, struct icon_info *item)
{
  ...
  while (tmp != NULL){
    if ((unsigned)tmp->mouse == d.xbutton.button && tmp->type == type){
