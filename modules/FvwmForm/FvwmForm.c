...
void dummy (UNUSED FILE *fp, UNUSED const char *fmt, ...) {
}
...
char *CopyStringL (char *cp, int n)
{
  char *dp, *bp;
  if (n == 0)
    n = strlen(cp);
...
#define AddToLine(item) { cur_line->items[cur_line->n++] = item; cur_line->size_x += item->header.size_x; if (cur_line->size_y < item->header.size_y) cur_line->size_y = item->header.size_y; }
  ...
  while (GetConfigLine(fd,&line_buf),line_buf) {
    ...
    if (strncmp(cp, "GrabServer", 10) == 0) {
      ...
      cp += 10;
      while (isspace(*cp)) cp++;
      font_names[f_button] = CopyStringL(cp, 0);
      ...
      cp += 9;
      while (isspace(*cp)) cp++;
      font_names[f_input] = CopyStringL(cp, 0);
...
void ReadXServer ()
{
  ...
  while (XEventsQueued(dpy, QueuedAfterReading)) {
    ...
    for (i = 0; i < n_items; i++) {
      item = items + i;
      if (event.xany.window == (unsigned long)item->header.win) {
	switch (event.type) {
	case Expose:
	  RedrawItem(item, 0);
	  break;
