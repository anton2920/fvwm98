  ...
  if(b!=UberButton)
    {
      ...
      fprintf(stderr,"0x%06zx(%ix%i@(%i,%i),0x%04x): ",
	      (uintptr_t)b,b->BWidth,b->BHeight,
	      buttonXPos(b,button),buttonYPos(b,button),b->flags);
    }
  else
    fprintf(stderr,"0x%06zx(%ix%i@,0x%04x): ",(uintptr_t)b,
	    b->BWidth,b->BHeight,b->flags);

  if(b->flags&b_Font)
    fprintf(stderr,"Font(%s,%zu) ",b->font_string,(uintptr_t)b->font);
...
*/
	...
	fprintf(stderr,"0x%06zx ",(uintptr_t)b->c->buttons[i++]);
      fprintf(stderr,"}\n");
      i=0;
      while(i<b->c->num_buttons)
